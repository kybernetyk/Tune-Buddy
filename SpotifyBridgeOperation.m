...
- (void) fetchCurrentTrack
{
	...
	SpotifyTrack *currentTrack = nil;
	
	@try
	{
//		currentTrack = //[[iTunes currentTrack] get];
		//currentTrack = [[spotify currentTrack] get];
	...
	//if ([currentTrack exists] && [spotify isRunning])
	if ([spotify isRunning])
	{
		@try 
		{
			...
			playbackPosition = [spotify playerPosition];
	...
	if (playerState == SpotifyEPlSPlaying)
	{	
		[self setPlayStatus: @"♫ "];
